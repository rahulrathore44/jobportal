...
public class TestJobPrortalControllerDeleteEndPoint {
	...
	@Test
	public void test_delete_end_point_json() throws Exception {
		mvc.perform(delete(NORMAL_PATH + "/remove/{id}", id)
		...
		mvc.perform(get(NORMAL_PATH + "/find").queryParam("id", String.valueOf(id)).queryParam("jobTitle",
				JOB_TITLE)).andExpect(status().isNotFound());
	}
	
	@Test
	public void test_delete_end_point_xml() throws Exception {
		mvc.perform(delete(NORMAL_PATH + "/remove/{id}", id)
	...
	@Test
	public void test_delete_end_point_when_id_notfound() throws Exception {
		mvc.perform(delete(NORMAL_PATH + "/remove/{id}", (id + 1))
				.accept(MediaType.APPLICATION_XML))
